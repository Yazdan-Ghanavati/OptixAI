...
import numpy as np
from Medical_Recommendations import cnv,dme,drusen,normal
import tempfile


# Move model loading outside and cache it
@st.cache_resource
def load_my_model():
    # Use the dummy F1Score fix we used earlier if the model still has that custom metric
    def F1Score(*args, **kwargs): return 0
    
    model = tf.keras.models.load_model(
        "Eye_Disease_Classification_Trained_Model.h5", 
...
#Prediction Page
    ...
    if(st.button("Predict")) and test_image is not None:
        with st.spinner("Please Wait.."):
            ...
            os.remove(temp_file_path)  # Clean up the temporary file after prediction
            #Reading Labels
        ...
        with st.expander("Learn More"):
            ...
            if(result_index==0):
                ...
                # cnv = "Generate a comprehensive recommendation for a patient diagnosed with CNV based on the OCT scan findings. " \
                # "The recommendation should include: " \
                # "1) Immediate referral to a retinal specialist, " \
                # "2) Treatment options such as anti-VEGF therapy, photodynamic therapy, or laser treatment, " \
                # "3) Lifestyle and monitoring advice including dietary recommendations and regular follow-up OCT scans, and " \
                # "4) Next steps for managing the condition effectively."
                # response = llm(cnv)
